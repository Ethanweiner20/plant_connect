...
class USDAPlantsTest < MiniTest::Test
  ...
  def test_common_name
    ...
    assert_equal 1, USDAPlants.search({ "CommonName" => "arizona Boxelder" }, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size
    assert_equal 2, USDAPlants.search({ "CommonName" => "Boxelder" }, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size
  end

  def test_no_filters
    assert_equal 0, USDAPlants.search({}, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size
  end
  ...
  def test_empty_filters
    ...
    assert_equal 1, USDAPlants.search(filters, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size
  ...
  def test_multiple_filters
    ...
    assert_equal 2, USDAPlants.search(filters, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size
  end
  ...
  def test_invalid_filters
    ...
    assert_raises(StandardError) { USDAPlants.search(filters, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size }
  end

  def test_search_limit
    filters = { "Genus" => "Acer" }
    assert_equal USDAPlants::SEARCH_LIMIT, USDAPlants.search(filters, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size
  end

  def test_multi_value_filter
    filters = { "Genus" => "Albizia", "GrowthHabit" => ["Tree", "Shrub"] }
    assert_equal 4, USDAPlants.search(filters, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size
  ...
  def test_range
    ...
    assert_equal 3, USDAPlants.search(filters, max_index: TEMPORARY_SEARCH_LIMIT)[:plants].size
